"""
State Transition Stress Test - OpenAI Models (GPT-4 / GPT-5 Family)
-------------------------------------------------------
Description:
    Evaluates both the probabilistic paradigm (GPT-4) and latent reasoning 
    architectures (GPT-5) to demonstrate how prosthetic memory mitigates, 
    but does not eliminate, asymptotic decay.
...
if __name__ == "__main__":
    ...
    output_file = f"results_openai_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
    
    with open(output_file, mode='w', newline='') as f:
        csv.writer(f).writerow(["Model", "L", "Iter", "Success"])
    
    target_models = ["gpt-4.1", "gpt-4.1-mini", "gpt-5.4", "gpt-5.4-mini"]
